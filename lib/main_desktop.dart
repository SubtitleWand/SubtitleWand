...
import 'package:flutter/foundation.dart'
    show debugDefaultTargetPlatformOverride, describeEnum;
import 'package:flutter/material.dart' hide Image;
import 'package:flutter/services.dart' show ByteData, FontLoader, LogicalKeyboardKey, RawKeyDownEvent, RawKeyUpEvent, RawKeyboard, TextInputFormatter, WhitelistingTextInputFormatter, rootBundle;
...
import 'package:subtitle_wand/main_page_bloc.dart' as MPB;
...
class _HomePageState extends State<HomePage> with SingleTickerProviderStateMixin {
  ...
  void initState() { 
    ...
    // Properties
    _paddingLeftTextCon = new TextEditingController(text: _bloc.state.propertyPaddingLeft.toString())..addListener((){
      _bloc.add(MPB.PropertyPaddingLeftEvent(int.tryParse(_paddingLeftTextCon.text) ?? 0));
    });
    _paddingRightTextCon = new TextEditingController(text: _bloc.state.propertyPaddingRight.toString())..addListener((){
      _bloc.add(MPB.PropertyPaddingRightEvent(int.tryParse(_paddingRightTextCon.text) ?? 0));
    });
    _paddingTopTextCon = new TextEditingController(text: _bloc.state.propertyPaddingTop.toString())..addListener((){
      _bloc.add(MPB.PropertyPaddingBottomEvent(int.tryParse(_paddingTopTextCon.text) ?? 0));
    });
    _paddingBottomTextCon = new TextEditingController(text: _bloc.state.propertyPaddingBottom.toString())..addListener((){
      _bloc.add(MPB.PropertyPaddingBottomEvent(int.tryParse(_paddingBottomTextCon.text) ?? 0));
    });
    // Properties - font
    _fontSizeTextCon = new TextEditingController(text: _bloc.state.propertyFontSize.toString())..addListener((){
      _bloc.add(MPB.PropertyFontSizeEvent(int.tryParse(_fontSizeTextCon.text) ?? 0));
    });
    // Properties - border
    _borderWidthTextCon = new TextEditingController(text: _bloc.state.propertyBorderWidth.toString())..addListener((){
      _bloc.add(MPB.PropertyBorderSizeEvent(int.tryParse(_borderWidthTextCon.text) ?? 0));
    });
    // Properties - shadow
    _offsetXTextCon = new TextEditingController(text: _bloc.state.propertyShadowX.toString())..addListener((){
      _bloc.add(MPB.PropertyShadowOffsetXEvent(int.tryParse(_offsetXTextCon.text) ?? 0));
    });
    _offsetYTextCon = new TextEditingController(text: _bloc.state.propertyShadowY.toString())..addListener((){
      _bloc.add(MPB.PropertyShadowOffsetYEvent(int.tryParse(_offsetYTextCon.text) ?? 0));
    });
    _spreadTextCon = new TextEditingController(text: _bloc.state.propertyShadowSpread.toString())..addListener((){
      _bloc.add(MPB.PropertyShadowSpreadEvent(int.tryParse(_spreadTextCon.text) ?? 0));
    });
    _blurTextCon = new TextEditingController(text: _bloc.state.propertyShadowBlur.toString())..addListener((){
    ...
    // Properties - Canvas
    _canvasResolutionXTextCon = new TextEditingController(text: _bloc.state.propertyCanvasResolutionX.toString())..addListener((){
      _bloc.add(MPB.PropertyCanvasResolutionXEvent(int.tryParse(_canvasResolutionXTextCon.text) ?? 0));
    });
    _canvasResolutionYTextCon = new TextEditingController(text: _bloc.state.propertyCanvasResolutionY.toString())..addListener((){
      _bloc.add(MPB.PropertyCanvasResolutionYEvent(int.tryParse(_canvasResolutionYTextCon.text) ?? 0));
    });
    // Properties - Text
    _subtilteTextController = new TextEditingController(text: "")..addListener((){
        ...
        child: Container(
          child: BlocListener<MPB.MainPageBloc, MPB.MainPageState>(
            ...
            listener: (context, state) {
              ...
              if(state.horizontalAlignment == SubtitleHorizontalAlignment.Left)
                align = TextAlign.left;
              if(state.horizontalAlignment == SubtitleHorizontalAlignment.Right)
                                      ...
                                      child: Container(
                                        ...
                                        child: MouseRegion(
                                          ...
                                          child: SubtitlePanel(
                                            ...
                                            canvasResolution: Size(state.propertyCanvasResolutionX.toDouble(), state.propertyCanvasResolutionY.toDouble()),
                                            span: TextSpan(
                                              text: "${state.propertySubtitleTexts.length > 0 ? state.propertySubtitleTexts[state.currentFrame] : ""}",
                                              style: TextStyle(
